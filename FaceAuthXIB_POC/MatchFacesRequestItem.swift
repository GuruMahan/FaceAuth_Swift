    ...
    @objc private func handleMatchButtonPress() {
        ...
        FaceSDK.service.matchFaces(request, completion: { (response: MatchFacesResponse) in
            ...
            if let error = response.error {
                self.similarityLabel.text = "SimilarityError: \(error.localizedDescription)"
                return
            }

            if let firstPair = response.results.first {
                let similarity = String(format: "%.5f", firstPair.similarity?.doubleValue ?? 0.0)
                if firstPair.similarity?.doubleValue ?? 0.0 > 0.8{
                    self.similarityLabel.text = "Face match successfully\n \(similarity)"
                }else{
                    self.similarityLabel.text = "Face not match\n \(similarity)"
                }
            } else {
